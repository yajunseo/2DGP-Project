...
class Barrier:
    ...
    def __init__(self, x, y, phase, number):
        if Barrier.image is None:
            Barrier.image = load_image('sprite\\Effect\\thunderEffect.png')
 ...
 #       draw_rectangle(*self.get_bb())
        self.image.clip_draw(int(self.frame) * 16 + 48, 0, 16, 16, self.x, self.y, 50, 50)

    def update(self):
        self.frame = (self.frame + FRAMES_PER_ACTION * ACTION_PER_TIME * game_framework.frame_time) % 3
        self.x = math.sin(self.dir * pi / 180) * 100
        self.y = math.cos(self.dir * pi / 180) * 100
